...
def test_create_business_defaults_opened_at_to_today_when_missing_everywhere(
    ...
    client = client_factory(full_name="Today Client", id_number="BTODAY1")
...
def test_update_business_rejects_invalid_status_value(test_db, client_factory, business_factory):
    client = client_factory(full_name="Additional Client", id_number="BADD001")
    business = _create_business_row(
        client_factory, business_factory, legal_entity_id=client.legal_entity_id
    )
    service = BusinessService(test_db)
...
def test_update_business_rejects_wrong_client_id(test_db, client_factory, business_factory):
    client = client_factory(full_name="Additional Client", id_number="BADD002")
    business = _create_business_row(
        client_factory, business_factory, legal_entity_id=client.legal_entity_id
    )
    service = BusinessService(test_db)
...
def test_update_business_to_active_clears_closed_at(
    test_db, client_factory, business_factory, actor_user
):
    client = client_factory(full_name="Additional Client", id_number="BADD003")
    business = _create_business_row(
        client_factory,
        business_factory,
...
def test_delete_business_raises_when_missing(test_db, actor_user):
    service = BusinessService(test_db)

    with pytest.raises(NotFoundError) as exc:
        service.delete_business(123, actor_id=actor_user.id)

    assert exc.value.code == "BUSINESS.NOT_FOUND"


def test_restore_business_requires_advisor_role(test_db, actor_user):
    service = BusinessService(test_db)

    with pytest.raises(ForbiddenError) as exc:
        service.restore_business(1, actor_id=actor_user.id, actor_role=UserRole.SECRETARY)

    assert exc.value.code == "BUSINESS.FORBIDDEN"


def test_restore_business_not_deleted_raises_conflict(test_db, actor_user):
    service = BusinessService(test_db)
    ...
    with pytest.raises(ConflictError) as exc:
        service.restore_business(1, actor_id=actor_user.id, actor_role=UserRole.ADVISOR)

    assert exc.value.code == "BUSINESS.NOT_DELETED"


def test_restore_business_raises_when_repo_restore_returns_none(test_db, actor_user):
    service = BusinessService(test_db)
    service._lifecycle.business_repo = SimpleNamespace(
    ...
    with pytest.raises(NotFoundError) as exc:
        service.restore_business(2, actor_id=actor_user.id, actor_role=UserRole.ADVISOR)

    assert exc.value.code == "BUSINESS.NOT_FOUND"


def test_restore_business_restores_soft_deleted_business_and_writes_audit(
    test_db, client_factory, business_factory, actor_user
):

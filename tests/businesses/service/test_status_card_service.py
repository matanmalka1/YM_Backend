...
def test_empty_client_returns_zero_counts(test_db, client_factory):
    client_id = _client(client_factory, "SC-EMPTY-001")
...
def test_vat_card_latest_period_is_lexicographic_max(test_db, client_factory, actor_user):
    client_id = _client(client_factory, "SC-VAT-002")
...
def test_charges_card_sums_issued_charges_only(test_db, client_factory, charge_factory):
    client_id = _client(client_factory, "SC-CH-001")
...
def test_binders_card_counts_active_and_in_office(
    test_db, client_factory, binder_factory, actor_user
):
    client_id = _client(client_factory, "SC-BN-001")
...
def test_documents_card_counts_present_separately(
    test_db, test_user, client_factory, permanent_document_factory
):
    client_id = _client(client_factory, "SC-DOC-001")
...
def test_year_defaults_to_current_year_when_not_provided(test_db, client_factory):
    from app.utils.time_utils import utcnow

    client_id = _client(client_factory, "SC-YR-001")

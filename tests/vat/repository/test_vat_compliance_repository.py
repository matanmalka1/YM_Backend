"""The SQL half of the shared resolved-status set.

``RESOLVED_OBLIGATION_STATUSES`` is read by both ``is_obligation_resolved``
and every SQL query asking the same question. tests/common/test_obligation_lifecycle.py
...
@pytest.mark.parametrize("status", sorted(RESOLVED_OBLIGATION_STATUSES, key=lambda s: s.value))
def test_resolved_statuses_are_excluded_from_overdue(test_db, user_factory, client_factory, status):
    ...
    """
    assert is_obligation_resolved(status)
...
def test_cancelled_period_is_not_overdue(test_db, user_factory, client_factory):
    ...
    _item(repo, client.id, user.id, "2026-01", ObligationStatus.CANCELED)
    open_item = _item(repo, client.id, user.id, "2026-02", ObligationStatus.AWAITING_INPUT)
    test_db.flush()
...
def test_unresolved_statuses_are_returned(test_db, user_factory, client_factory, status):
    assert not is_obligation_resolved(status)
...
def test_soft_deleted_period_is_excluded(test_db, user_factory, client_factory):
    ...
    item = _item(repo, client.id, user.id, "2026-01", ObligationStatus.AWAITING_INPUT)
